...
from ed_companion.module_identity import module_identity_key, same_module_identity
...
class ModulePurchaseDisplayTests(unittest.TestCase):
    ...
    def test_every_known_module_alias_uses_one_global_identity(self):
        aliases = (
            ("ship_armour_grade1", "lakonminer_armour_grade1"),
            ("ship_armour_grade1", "explorer_nx_armour_grade1_default"),
            ("ship_armour_reactive", "krait_mkii_armour_reactive"),
            ("ship_armour_reactive", "ferdelance_armour_reactive"),
            (
                "hpt_heatsinklauncher_turret_tiny",
                "$Hpt_HeatSinkLauncher_Tiny_Name;",
            ),
            (
                "hpt_miningtoolv2_fixed_large",
        ...
        for canonical, alias in aliases:
            with self.subTest(alias=alias):
                self.assertTrue(same_module_identity(canonical, alias))
                self.assertEqual(
                    module_identity_key(canonical), module_identity_key(alias)
                )

        self.assertFalse(same_module_identity(
            "ship_armour_reactive", "ship_armour_mirrored"
        ))
        self.assertFalse(same_module_identity(
            "hpt_pulselaser_gimbal_large", "hpt_pulselaser_fixed_large"
        ))

    def test_catalog_wide_wrapped_module_ids_keep_the_same_identity(self):
        import json
        path = Path(__file__).resolve().parents[1] / "ed_data/module_display.json"
        catalog = json.loads(path.read_text(encoding="utf-8"))["modules"]
        ...
        for symbol in catalog:
            with self.subTest(symbol=symbol):
                self.assertTrue(same_module_identity(
                    symbol, f"${symbol}_Name;"
                ))
