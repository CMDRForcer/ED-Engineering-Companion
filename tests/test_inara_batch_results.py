...
class InaraBatchResultTests(unittest.TestCase):
    ...
    def test_mixed_batch_retires_accepted_and_permanently_rejected_events(self):
        ...
        self.assertEqual(controller._inara_pending_fingerprints, [])
        self.assertEqual(
            controller._inara_cache["fingerprints"],
            ["fingerprint-0", "fingerprint-1", "fingerprint-2"],
        )
        self.assertEqual(len(controller._inara_pending_events), 0)
        self.assertIn("partially accepted", controller._inara_status)
        self.assertEqual(
            controller._inara_receipts[0]["detail"],
            "2 accepted; 1 permanently rejected",
        )

    def test_retryable_partial_failure_remains_pending(self):
        controller = _controller(2)
        receipt = {
            "httpStatus": 200, "elapsedMs": 10,
            "acceptedIndexes": [0], "failedIndexes": [1],
            "retryableFailedIndexes": [1],
        }
        CockpitController._finish_inara(controller, {
            "context": dict(controller._active_inara_request),
            "operation": "journal", "success": True,
            "message": json.dumps(receipt), "ships": {},
        })
        self.assertEqual(controller._inara_pending_fingerprints, ["fingerprint-1"])
        self.assertEqual(controller._inara_cache["fingerprints"], ["fingerprint-0"])
        self.assertEqual(
            controller._inara_receipts[0]["detail"],
            "1 accepted; 1 retained for retry",
        )
