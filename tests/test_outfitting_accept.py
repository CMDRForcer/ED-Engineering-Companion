...
class OutfittingAcceptTests(unittest.TestCase):
    ...
    def test_accept_removes_only_blueprint_bound_to_replacement(self):
        with TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "desired_outfitting.json").write_text(json.dumps({
                "39": {"TinyHardpoint2": "hpt_shieldbooster_size0_class5"},
            }), encoding="utf-8")
            matching = [{"_Planner": {
                "plan_id": "accepted-plan", "ship_id": "39",
                "slot": "TinyHardpoint2",
                "module_id": "hpt_shieldbooster_size0_class5",
            }}]
            child = [{
                "Kind": "ExperimentalEffect", "_ParentPlanId": "accepted-plan",
            }]
            unrelated = [{"_Planner": {
                "plan_id": "other-plan", "ship_id": "39",
                "slot": "TinyHardpoint3",
                "module_id": "hpt_shieldbooster_size0_class5",
            }}]
            same_slot_current = [{"_Planner": {
                "plan_id": "installed-plan", "ship_id": "39",
                "slot": "TinyHardpoint2",
                "module_id": "hpt_heatsinklauncher_turret_tiny",
            }}]
            blueprint_path = root / "ship_blueprints.json"
            blueprint_path.write_text(json.dumps({
                "Lakonminer": [
                    matching, child, unrelated, same_slot_current,
                ],
            }), encoding="utf-8")
            controller = CockpitController.__new__(CockpitController)
            controller._data_dir = root
            controller._selected_ship = "Lakonminer"
            controller._state = {
                "selectedShipId": "39",
                "engineeringShipSlots": [{
                    "slot": "TinyHardpoint2",
            ...
            saved = json.loads(blueprint_path.read_text(encoding="utf-8"))
            self.assertEqual(
                saved["Lakonminer"], [unrelated, same_slot_current]
            )
            self.assertEqual(
                json.loads((root / "desired_outfitting.json").read_text(
                    encoding="utf-8"
                )),
                {},
            )
