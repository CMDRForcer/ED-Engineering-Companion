...
powershell -NoProfile -Command "Compress-Archive -Path 'dist\EDEC\*' -DestinationPath 'output\EDEC-21.167-Windows.zip' -CompressionLevel Optimal -Force"
if errorlevel 1 goto :failed

echo.
echo Portable build created in dist\EDEC
echo Windows Explorer compatible ZIP created in output\EDEC-21.167-Windows.zip
exit /b 0
