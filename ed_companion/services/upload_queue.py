...
def partition_upload_queue(jobs, sent_limit=100):
    """Split old sent receipts from active delivery work without losing either."""
    valid = [deepcopy(item) for item in jobs or [] if isinstance(item, dict)]
    sent = [item for item in valid if item.get("status") == "sent"]
    pending = [item for item in valid if item.get("status") != "sent"]
    keep_count = max(0, int(sent_limit))
    if not keep_count:
        return pending, sent
    return sent[-keep_count:] + pending, sent[:-keep_count]
