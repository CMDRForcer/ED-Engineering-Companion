...
def apply_system_bgs_snapshot_batch(observations, snapshots, limit=10000):
    ...
    return (
        updated if limit is None else updated[-max(1, int(limit)):],
        len(accepted),
    )
...
def merge_hge_observation_batch(observations, additions, limit=10000):
    ...
    return updated if limit is None else updated[-max(1, int(limit)):], True
...
def partition_hge_observations(observations, now=None, max_age_seconds=86400):
    """Return active and historical rows so expiry never destroys evidence."""
    now = now or datetime.now(timezone.utc)
    kept, historical = [], []
    for row in observations or []:
        if row.get("self_test"):
        ...
        expired = expired_signal or (
            age is not None and age > max_age_seconds
        )
        (historical if expired else kept).append(row)
    return kept, historical
