...
MINING_COMMODITY_NAMES = {
    f"${identifier}_name;": row["name"]
    for identifier, row in MINING_COMMODITIES.items()
}
...
def mining_commodity_display_name(symbol: Any, localized: Any = "") -> str:
    ...
    known = MINING_COMMODITIES.get(mining_commodity_id(key))
    return str(known["name"]) if known else key or "Unknown mineral"
